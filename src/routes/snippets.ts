...
snippetRouter.get("/:user_id/:snippet_id", getSnippet);//get snippet by user and snippet id using dynamic routes

async function getSnippet(req: Request, res: Response) {
  async (req: Request, res: Response) => {
    const { user_id, snippet_id } = req.params;  
    if (!user_id || isNaN(parseInt(user_id))) {//beginning of validation for user_id and snippet_id
    ...
    try {
      const snippet = await pool.query(//parameterized queries are used to prevent SQL injection attacks.
      ...
      res.json(snippet.rows[0]);//sends the data of the found snippet back to the client as a JSON object.
    } catch (err) {
      console.error(err);
...
async function createSnippet(req: Request, res: Response) {
  ...
  if (!title) {//beginning of validation
